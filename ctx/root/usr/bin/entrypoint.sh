...
function set_permissions() {
    ...
    addgroup -S -g $CADDY_GID $CADDY_USER && adduser -h /var/lib/$CADDY_USER -S -u $CADDY_UID $CADDY_USER -G $CADDY_USER

    echo "Setting permissions for $CADDY_USER on /var/lib/$CADDY_USER"
    chown -R $CADDY_USER:$CADDY_USER /var/lib/$CADDY_USER

    if [ -n "$XDG_DATA_HOME" ]; then
        echo "Setting permissions for $CADDY_USER on XDG_DATA_HOME: $XDG_DATA_HOME"
        chown -R $CADDY_USER:$CADDY_USER $XDG_DATA_HOME
    fi
    if [ -n "$XDG_CONFIG_HOME" ]; then
        echo "Setting permissions for $CADDY_USER on XDG_CONFIG_HOME: $XDG_CONFIG_HOME"
        chown -R $CADDY_USER:$CADDY_USER $XDG_CONFIG_HOME
    fi
